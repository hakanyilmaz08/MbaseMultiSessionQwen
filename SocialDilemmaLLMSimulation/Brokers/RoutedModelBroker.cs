...
public sealed class RoutedModelBrokerOptions
{
...
public sealed class RoutedModelBroker : IModelBroker
{
    private readonly IHttpClientFactory _http;
    private readonly ILogger<RoutedModelBroker> _log;
    private readonly RoutedModelBrokerOptions _opt;

    public RoutedModelBroker(IHttpClientFactory http, IOptions<RoutedModelBrokerOptions> opt, ILogger<RoutedModelBroker> log)
    {
        _http = http;
        _opt = opt.Value;
        _log = log;
      ...
      double temperature, double topP, string? kvHandle, bool bearerAuth, CancellationToken ct)
    {
        ...
        var payload = new Dictionary<string, object?>
        {
            ...
            ["top_p"] = topP,
            ["stream"] = false,
            ["max_tokens"] = route.MaxTokens ?? _opt.DefaultMaxTokens
        };
        if (route.Reasoning?.HasValue == true)
            payload["reasoning"] = route.Reasoning.ToPayload();
